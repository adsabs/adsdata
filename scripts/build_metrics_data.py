...
class Builder(Process):
    ...
    def run(self):
        ...
        while True:
            ...
            try:
                doc = self.session.generate_metrics_data(bibcode)
                self.session.store_metrics_data(doc)
...
def build_synchronous(opts):
    ...
    for bib in get_bibcodes(opts):
        doc = session.generate_metrics_data(bib)
        if doc is not None:
            saved = session.store_metrics_data(doc)
...
if __name__ == "__main__":
    ...
    op = OptionParser()
    op.set_usage("usage: build_metrics_data.py [options] [%s]" % '|'.join(commands.map.keys()))
